...
const ProductsCard = ({title, image, subt}) => {
  return (
    <div className="relative rounded-3xl h-[300px] w-[300px] shadow-xl overflow-hidden group">
     
     <div  className="h-full w-full bg-cover  bg-center duration-500 group-hover:scale-110"
  ...
  return(
    <div className="flex flex-row px-40 gap-10">
    {
        items.map((item, index) =>(
          <ProductsCard key={index} title={item.title} image={item.image} subt={item.subt}></ProductsCard>
